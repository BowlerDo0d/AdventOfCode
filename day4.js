...
input.forEach(function (room) {
    var pieces = room.split('-'),
        sector_and_checksum = pieces.pop(),
        matches = sector_and_checksum.match(/(.+)(\[.+\])/),
        sector = parseInt(matches[1], 10),
        checksum = matches[2].replace('[', '').replace(']',''),
        sortedChecksum = checksum.split('').sort().join(''),
        charString = pieces.join(''),
        chars = [],
        fiveCommon = null,
        fiveCommonStr = '';
    ...
    for (var i = 0; i < charString.length; i++) {
        var char = charString[i];
        var charFound = findInArray(chars, char);

        if (charFound > -1) {
            chars[charFound].count += 1;
        } else {
            chars.push({ char: char, count: 1 });
        }
    }
    ...
    fiveCommon = chars.slice(0, 5).sort(function(a, b) {
        if (a.char < b.char) return -1;
        if (a.char > b.char) return 1;
        return 0;
    });

    fiveCommon.forEach(function (common) {
        fiveCommonStr += common.char;
    });

    log(fiveCommonStr);

    if (fiveCommonStr === sortedChecksum) {
        realRooms.push({
            name: pieces,
            sectorId: sector,
            checksum: checksum,
            sortedChecksum: sortedChecksum
        });
        log('Real room: ' + room);
        totalSector += sector;
    }
...
realRooms.forEach(function (room) {
    var shiftCount = room.sectorId % alphabet.length,
        realName = '';

    room.name.forEach(function (encryptedWord) {
        var word = '';

        for (var k = 0; k < encryptedWord.length; k++) {
            var newLetterIndex = alphabet.indexOf(encryptedWord[k]) + shiftCount;

            if (newLetterIndex > alphabet.length - 1) {
                newLetterIndex = 0 + (newLetterIndex - alphabet.length);
            }

            word += alphabet[newLetterIndex];
        }
