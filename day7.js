...
function stringMatchesPattern(str, pattern) {
    ...
    if (pattern === 'abba') {
        if (str.length === 4) {
            ...
            matches = (str[0] !== str[1] && part1 === part2);
        }
    } else if (pattern === 'aba' || pattern === 'bab') {
        if (str.length === 3) {
            matches = (str[0] === str[2] && str[0] !== str[1]);
        }

        if (matches && pattern === 'bab') {
            log('A: ' + str[1]);
            log('B: ' + str[0]);
            part2A = str[1];
            part2B = str[0];
            part2Strings.push(part2A + part2B + part2A);
        }
    }
...
function stringContainsPattern(str, pattern) {
    ...
    for (var i = 0; i < str.length; i++) {
        if ((i + pattern.length) <= str.length && stringMatchesPattern(str.substr(i, pattern.length), pattern)) {
            contains = true;
        }
    }
...
//     var bracketMatches = ip.match(/\[[a-z]+\]/g),
//         invalidIP = false,
//         leftoverIP = ip;
//
//     bracketMatches.forEach(function (str) {
//         leftoverIP = leftoverIP.replace(str,'%');
//         str = str.replace('[','').replace(']','');
//         if (stringContainsPattern(str, 'abba')) {
//             invalidIP = true;
//         }
//     });
//
//     if (!invalidIP) {
//         leftoverIP.split('%').some(function (str) {
//             if (stringContainsPattern(str, 'abba')) {
//                 totalValid += 1;
//                 return true;
//             }
...
input.forEach(function (ip) {
    var bracketMatches = ip.match(/\[[a-z]+\]/g),
        ...
        part2Strings.length = 0;

        bracketMatches.forEach(function (str) {
            log('Checking string: ' + str);
            ...
            leftoverIP = leftoverIP.replace(str,'%');
            log('leftoverIP: ' + leftoverIP);
            str = str.replace('[','').replace(']','');
            stringContainsPattern(str, 'bab');
        });

        log(part2Strings);

        part2Strings.some(function (str) {
            if (leftoverIP.indexOf(str) > -1) {
                log(ip);
                totalValid += 1;
                return true;
            }
